...
python ingestion/scraper.py --city İstanbul --category Meyve
```
...
Output: `ingestion/data/{şehir}_{ilçe}/{kategori}.jsonl` — one product JSON per line.
State: `state.json` at project root tracks last-scraped timestamps.

### Adding a new city
Edit `ingestion/config.py` → `CITIES` dict only. No other file changes needed.

### Ingestion modules
- `ingestion/config.py` — cities, categories, API constants, timing settings
- `ingestion/client.py` — raw HTTP calls (coordinates, depots, paginated product fetch)
- `ingestion/state.py` — load/save/check staleness of `state.json`
- `ingestion/scraper.py` — orchestration entrypoint
- `ingestion/depot_grid.py` — grid search to build a complete depot database (see below)
...
İlçe merkezi koordinatı
    ...
    → JSONL: ingestion/data/{şehir}_{ilçe}/{tarih}/{kategori}.jsonl
```
