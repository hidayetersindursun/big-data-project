...
DEPOT_RADIUS_KM = 10         # Radius to search for nearby depots
...
# Proxy support
# - MFR_PROXY env var: "http://user:pass@host:port" formatı
# - MFR_PROXY_LIST env var: virgülle ayrılmış birden fazla proxy URL (round-robin)
PROXY = os.getenv("MFR_PROXY")
PROXY_LIST = [p.strip() for p in (os.getenv("MFR_PROXY_LIST") or "").split(",") if p.strip()] or None
...
CITIES = {
    ...
    "Düzce": [
        "Akçakoca", "Cumayeri", "Çilimli", "Gökçebey", "Gölyaka",
...
# API'ye gönderilen gerçek keyword (display adından farklı olabilir)
CATEGORY_API_KEYWORD = {
    "Meyve_Sebze": "Meyve",  # API "Meyve ve Sebze"yi tek kategori olarak döndürür
}
