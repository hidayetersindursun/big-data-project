...
MD_MACRO = """### Makro Etkenler & Gıda Fiyatı Korelasyonu

Dış maliyet ve makroekonomik göstergelerin gıda fiyatı/marjı ile **gecikmeli (lag) korelasyonu**.

- **akaryakit** — yakıt fiyatı (ulaşım / lojistik maliyeti)
- **tcmb** — USD/TRY kuru, TÜFE-gıda, ticari kredi faizi
- **commodities** — global emtia (buğday, brent — öncü referans fiyat)
- **epias** — elektrik piyasa fiyatı (üretim / soğuk-zincir maliyeti)

`corr_basis = change` (fark serisi) birincil yorumlanmalı — ham seviye (`level`) trendli serilerde yanıltıcı korelasyon verir. `best_lag`, her seri-ürün çifti için en güçlü gecikmeyi işaretler."""
...
def build_all(dv):
    ...
    # === Dashboard 5: Makro Etkenler & Gıda Fiyatı Korelasyonu ===
    mc = dv["gidaradar_macro_corr"]
    objs += [
        lens_metric("gr-l-mc-kpi", "En Yüksek Pozitif Korelasyon", mc,
                    col_metric("max", "correlation", "Maks. korelasyon", 3)),
        lens_xy("gr-l-mc-source", "Kaynak Bazlı Ortalama Korelasyon", mc,
                col_terms("macro_source", 4, order_col="m1"),
                {"m1": col_metric("average", "correlation", "Ort. korelasyon", 3)},
                series_type="bar"),
        lens_xy("gr-l-mc-series", "Makro Seri Bazlı Ortalama Korelasyon", mc,
                col_terms("macro_series", 15, order_col="m1"),
                {"m1": col_metric("average", "correlation", "Ort. korelasyon", 3)},
                series_type="bar_horizontal"),
        lens_table("gr-l-mc-table", "Korelasyon Detayı (lag bazlı)", mc,
                   {"c1": col_terms("macro_series", 50, order_col="c6"),
                    "c2": col_terms("product_canonical", 50),
                    "c3": col_terms("target_type", 5),
                    "c4": col_terms("corr_basis", 5),
                    "c5": col_terms("lag_days", 61),
                    "c6": col_metric("average", "correlation", "Korelasyon", 3),
                    "c7": col_metric("average", "p_value", "p-değeri", 4)},
                   ["c1", "c2", "c3", "c4", "c5", "c6", "c7"]),
    ]
    objs.append(dashboard("gr-dashboard-macro", "GıdaRadar — Makro Etkenler", [
        ("gr-l-mc-kpi", 0, 0, 48, 6),
        ("gr-l-mc-source", 0, 6, 24, 14),
        ("gr-l-mc-series", 24, 6, 24, 14),
        ("gr-l-mc-table", 0, 20, 48, 18),
    ], intro_md=MD_MACRO))
...
def main():
    ...
    for need in ("gidaradar_daily_margin", "gidaradar_rockets_feathers",
                 "gidaradar_shocks", "gidaradar_forecast", "gidaradar_macro_corr"):
        if need not in dv:
